      ...
      <div className="mx-auto max-w-[1400px] space-y-5 animate-in fade-in duration-300">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">Track, triage and resolve operational incidents.</p>
          </div>
              ...
              <TableBody>
                {filtered.map((inc) => (
                  <TableRow
                    ...
                    onClick={() => navigate({ to: "/admin/incidents/$id", params: { id: inc.id } })}
                  >
                    <TableCell>
                      <span className="font-mono text-xs text-muted-foreground">{inc.refNumber}</span>
                    </TableCell>
                    <TableCell>
                      <p className="font-medium text-foreground text-sm leading-snug line-clamp-1">{inc.title}</p>
                      {inc.location && (
                        <p className="text-xs text-muted-foreground mt-0.5">{inc.location}</p>
                      )}
                    </TableCell>
                    <TableCell>
                      <span className={cn("inline-flex items-center rounded-full border px-2 py-0.5 text-[10px] font-medium whitespace-nowrap", STATUS_COLOR[inc.status] ?? "bg-gray-500/10 text-gray-600 border-gray-500/20")}>
                        {inc.status.replace(/_/g, " ")}
                      </span>
                    </TableCell>
                    <TableCell>
                      <span className={cn("inline-flex items-center rounded-md px-2 py-0.5 text-[10px] font-semibold", SEVERITY_COLOR[inc.severity] ?? "bg-gray-500/10 text-gray-600")}>
                        {inc.severity}
                      </span>
                    </TableCell>
                    <TableCell className="hidden md:table-cell">
                      <span className="text-xs text-muted-foreground">{inc.type?.replace(/_/g, " ")}</span>
                    </TableCell>
                    <TableCell className="hidden lg:table-cell">
                      <span className="text-xs text-foreground/80">{inc.site?.name ?? "—"}</span>
                    </TableCell>
                    <TableCell className="hidden lg:table-cell">
                      <span className="text-xs text-foreground/80">
                        {inc.reportedBy ? `${inc.reportedBy.firstName} ${inc.reportedBy.lastName}` : "—"}
                      </span>
                    </TableCell>
                    <TableCell className="hidden sm:table-cell">
                      <span className="text-xs text-muted-foreground">{new Date(inc.occurredAt).toLocaleDateString()}</span>
                    </TableCell>
                    <TableCell>
