...
export function LoginForm() {
  ...
  const onSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      if (!APP_ROLES.includes(role)) {
        ...
        setErrors({ form: "Access denied. This portal is for Managers and Staff only." });
        return;
      }
      brandingStore.invalidate();
      brandingStore.fetch();
      navigate({ to: (search.redirect?.startsWith("/app") ? search.redirect : "/app/dashboard") as any });
    } catch (e: unknown) {
      const msg =
  ...
  const onDemoLogin = async (demo: typeof DEMO_ROLES[number]) => {
    ...
    try {
      await authStore.login(demo.email, demo.password, demo.tenantSlug);
      brandingStore.invalidate();
      brandingStore.fetch();
