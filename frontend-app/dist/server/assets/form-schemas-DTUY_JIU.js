import { R as React, L as jsxRuntimeExports } from "./worker-entry-hKZhhGaI.js";
import { w as cn, Q as objectType, W as stringType, K as enumType, t as booleanType, r as arrayType } from "./router-BNkFluS9.js";
...
function Form({
  ...
  const errorList = Object.entries(errors).map(([k, v]) => ({ key: k, message: v?.message })).filter((e) => e.message);
  return /* @__PURE__ */ jsxRuntimeExports.jsx(FormProvider, { ...form, children: /* @__PURE__ */ jsxRuntimeExports.jsxs("form", { onSubmit: form.handleSubmit(onSubmit), className: cn("space-y-6", className), noValidate: true, children: [
    showSummary && errorList.length > 0 && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "rounded-xl border border-destructive/40 bg-destructive/5 px-4 py-3 text-xs text-destructive", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "font-medium", children: "Please fix the following:" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("ul", { className: "mt-1 list-disc pl-4", children: errorList.map((e) => /* @__PURE__ */ jsxRuntimeExports.jsx("li", { children: e.message }, e.key)) })
    ] }),
    typeof children === "function" ? children(form) : children
  ] }) });
}
function FormFieldShell({ name, label, description, error, children, className }) {
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: cn("space-y-2", className), children: [
    ...
    children,
    description && !error && /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "px-1 text-xs text-muted-foreground", children: description }),
    error && /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "px-1 text-xs font-medium text-destructive", children: error })
  ] });
}
function useFieldError(name) {
  const { formState } = useFormContext();
  const err = name.split(".").reduce((acc, key) => {
...
function normalize(opts) {
  return opts.map((o2) => typeof o2 === "string" ? { label: o2, value: o2 } : o2);
}
function FormSelect({
  name,
  label,
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsx(FormFieldShell, { name, label, description, error, className, children: /* @__PURE__ */ jsxRuntimeExports.jsxs(
    "select",
    {
      id: name,
      disabled,
      ...
      children: [
        ...
        opts.map((o2) => /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: o2.value, children: o2.label }, o2.value))
      ]
    }
  ) });
}
      ...
      children: [
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs(
          "button",
          {
            ...
            children: [
              submitting ? /* @__PURE__ */ jsxRuntimeExports.jsx(LoaderCircle, { className: "h-4 w-4 animate-spin" }) : icon,
...
objectType({
  title: stringType().trim().min(4, "Title is too short").max(120, "Too long"),
  description: stringType().trim().min(1, "Description is required").max(2e3),
  assignee: stringType().min(1, "Select an assignee"),
  dueDate: stringType().min(1, "Select a due date"),
  priority: enumType(["Low", "Medium", "High"])
});
objectType({
  title: stringType().trim().min(4, "Title is too short").max(120),
  description: stringType().trim().min(1, "Description is required").max(2e3),
  facility: stringType().min(1, "Select a facility"),
  reportedBy: stringType().min(1, "Select a reporter"),
  priority: enumType(["Low", "Medium", "High", "Critical"]),
  attachments: arrayType(stringType())
});
const facilitySchema = objectType({
  name: stringType().trim().min(1, "Facility name is required").max(120),
  type: enumType(["Office", "Plant", "Warehouse"], { message: "Select a type" }),
  location: stringType().trim().min(1, "Location is required").max(160),
  manager: stringType().min(1, "Select a manager"),
  status: enumType(["Active", "Under Maintenance"]),
  notes: stringType().max(2e3)
});
const companySchema = objectType({
  name: stringType().trim().min(1, "Company name is required").max(120),
  industry: stringType().min(1, "Select an industry"),
  plan: enumType(["Basic", "Pro", "Enterprise"]),
...
objectType({
  ...
  role: enumType(["Admin", "Manager", "User"]),
  status: enumType(["Active", "Disabled"])
});
const profileSchema = objectType({
  fullName: stringType().trim().min(1, "Full name is required").max(120),
  phone: stringType().trim().max(40)
});
const preferencesSchema = objectType({
  language: enumType(["en", "es", "fr", "de", "pt"]),
  timezone: stringType().min(1)
});
const notificationsSchema = objectType({
  auditUpdates: booleanType(),
  incidentAlerts: booleanType(),
  maintenanceReminders: booleanType(),
  emailNotifications: booleanType()
});
export {
  Controller as C,
  Form as F,
  ...
  profileSchema as g,
  useFormContext as h,
  notificationsSchema as n,
  preferencesSchema as p,
  useFieldError as u
};
