...
const QUICK_ACTIONS: Action[] = [
  { id: "qa-audit",    label: "Create Audit",     hint: "Start a new audit",     icon: ClipboardCheck, to: "/admin/audits/create" },
  { id: "qa-incident", label: "Report Incident",  hint: "Log a new incident",    icon: ShieldAlert,    to: "/admin/incidents/create" },
  { id: "qa-company",  label: "Add Company",      hint: "Create a new tenant",   icon: Users,          to: "/admin/tenants" },
];
...
async function runSearch(q: string): Promise<SearchItem[]> {
  ...
  if (incidents.status === "fulfilled")
    for (const inc of (incidents.value.data?.data ?? []))
      out.push({ id: inc.id, category: "Incidents", title: `${inc.refNumber} · ${inc.title}`, subtitle: [inc.location ?? inc.site?.name, inc.status].filter(Boolean).join(" · "), to: "/admin/incidents" });
  if (audits.status === "fulfilled")
    for (const aud of (audits.value.data?.data ?? []))
      out.push({ id: aud.id, category: "Audits", title: `${aud.refNumber} · ${aud.title}`, subtitle: [aud.site?.name, aud.status].filter(Boolean).join(" · "), to: "/admin/audits" });
  if (sites.status === "fulfilled")
    for (const site of (sites.value.data?.data ?? []))
      out.push({ id: site.id, category: "Facilities", title: site.name, subtitle: [site.type, site.status].filter(Boolean).join(" · "), to: "/admin/sites" });
  if (customers.status === "fulfilled")
    for (const c of (customers.value.data?.data ?? []))
      out.push({ id: c.id, category: "Companies", title: c.name, subtitle: [c.industry, c.status].filter(Boolean).join(" · "), to: "/admin/customers" });
  return out;
}
